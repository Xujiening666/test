...
BaseType_t osQueueSend(QueueHandle_t xQueue, const void * const pvItemToQueue, uint32_t ms);
BaseType_t osQueueReceive(QueueHandle_t xQueue, void * const pvBuffer, uint32_t ms);
BaseType_t osSemaphoreGive(QueueHandle_t xSemaphore);
BaseType_t osSemaphoreTake(QueueHandle_t xSemaphore, uint32_t ms);
BaseType_t osTaskNotify(TaskHandle_t xTaskToNotify, uint32_t ulValue, eNotifyAction eAction);

void vPreSleepHook(uint32_t para);
void vPostSleepHook(uint32_t para);
